...
class WooCommerce_Edit_Order_Numbers_Admin_Misc {
	...
	public function admin_notice() {
		...
		_e( 'Thanks for installing WooCommerce Edit Order Numbers! For more WooCommerce goodness, check out the ', WOOCOMMERCE_EDIT_ORDER_NUMBERS_TEXT_DOMAIN );

		printf( '<a href="%s" target="_blank">%s</a> | <a href="%s">%s</a>', esc_url( WOOCOMMERCE_EDIT_ORDER_NUMBERS_ULTIMATEWOO_URL ), __( 'most powerful WooCommerce plugin yet.', WOOCOMMERCE_EDIT_ORDER_NUMBERS_TEXT_DOMAIN ), esc_url( wp_nonce_url( '?uw_eon_ultimatewoo_alert=1', 'uw_eon_ultimatewoo_alert' ) ), __( 'Dismiss', WOOCOMMERCE_EDIT_ORDER_NUMBERS_TEXT_DOMAIN ) );
	...
	public function dismiss_admin_notice() {
		...
		if ( isset( $_GET['uw_eon_ultimatewoo_alert'] ) && intval( $_GET['uw_eon_ultimatewoo_alert'] ) === 1 ) {
			...
			if ( ! wp_verify_nonce( $_GET['_wpnonce'], 'uw_eon_ultimatewoo_alert' ) ) {
				wp_die( 'Dismiss failed.', WOOCOMMERCE_EDIT_ORDER_NUMBERS_TEXT_DOMAIN );
			}
